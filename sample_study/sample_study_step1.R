...
for(iter in 1:5){
  ...
  write.csv(metadata,
            paste0(path_base,"/iter",iter,"_metadata_SS.csv"))
  
  batch_df <- as.matrix(counts(sim_batch))
  nobatch_df <- as.matrix(counts(sim_noBatch))
  
  write.csv(batch_df,
            paste0(path_base,"/iter",iter,"_batch_df_SS.csv"))
  write.csv(nobatch_df,
            paste0(path_base,"/iter",iter,"_nobatch_df_SS.csv"))
  ...
  write.csv(count_batch_transformed$E,
            paste0(path_base,"/iter",iter,"_countmat_batch_transformed_SS.csv"))
  ...
  write.csv(covmat,
            paste0(path_base,"/covmat_SS.csv"))
  ...
  write.csv(combatseq_df,
            paste0(path_base,"/iter",iter,"_combatseq_df_SS.csv"))
  write.csv(recombatseq_df,
            paste0(path_base,"/iter",iter,"_recombatseq_df_SS.csv"))
  write.csv(recombatseq_df_parallel,
            paste0(path_base,"/iter",iter,"_recombatseq_parallel_df_SS.csv"))
  ...
  saveRDS(de_ground_truth, 
          file = paste0(path_base,"/iter",iter,"_DEgenes_SS.rds"))
...
#### save results
write.csv(res_df,
          paste0(path_base,"/results_SS.csv"))
